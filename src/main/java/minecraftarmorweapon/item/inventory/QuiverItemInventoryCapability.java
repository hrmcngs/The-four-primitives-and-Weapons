...
public class QuiverItemInventoryCapability implements ICapabilitySerializable<CompoundTag> {
	@SubscribeEvent
	@OnlyIn(Dist.CLIENT)
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntity().getItem().getItem() == MinecraftArmorWeaponModItems.QUIVER_ITEM.get()) {
	...
	private ItemStackHandler createItemHandler() {
		return new ItemStackHandler(9) {
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				return stack.getItem() != MinecraftArmorWeaponModItems.QUIVER_ITEM.get();
			}
