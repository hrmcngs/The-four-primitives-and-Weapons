...
public class KatanaTobuEntity extends AbstractArrow implements ItemSupplier {
	...
	public KatanaTobuEntity(EntityType<? extends KatanaTobuEntity> type, double x, double y, double z, Level world) {
		super(type, x, y, z, world);
	}

	public KatanaTobuEntity(EntityType<? extends KatanaTobuEntity> type, LivingEntity entity, Level world) {
	...
	public static KatanaTobuEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		KatanaTobuEntity entityarrow = new KatanaTobuEntity(MinecraftArmorWeaponModEntities.KATANA_TOBU.get(), entity, world);
