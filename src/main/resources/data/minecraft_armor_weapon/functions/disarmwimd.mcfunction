...
execute at @s[tag=!Loki_Disarm0,tag=!Loki_Disarm_Return] run function minecraft_armor_weapon:move
execute as @e[type=item,distance=..2] at @s run tp @s @e[tag=Loki_Disarm,limit=1,sort=nearest]

#戻ってくる
#execute if entity @s[scores={BulletRemain=5..}] at @s if entity @a[distance=..2] run tag @s add Loki_Disarm_Return
...
#ヒット
execute if entity @s[tag=!Loki_Disarm_Return,scores={BulletRemain=5..}] as @e[distance=..2,type=!item,tag=!Loki_Disarm,type=!area_effect_cloud] at @s run function minecraft_armor_weapon:disarm
