...
新規エクスポートでは、Lispが毎回新しいシードから4つの撮影エリアを作り、mcfunctionへ変換します。各エリアは96×96ブロック。滑らかな丘・曲がる水路・花の群生・枝垂れ桜・崩壊ビル・剣の配置が変わります。元の中央庭園は保持します。
生成は256コマンドずつ、4エリアを順番に実行するため数十秒以上かかります。完成メッセージを待ってから下記のcamera関数で移動してください。

生成方法・シード指定は `bash sh/generate/generate-photo-world.sh help` を参照してください。シードはワールド内の `datapacks/blade_gallery/terrain-seed.txt` に保存されます。
`/function blade_gallery:random/setup` は同じ地形を再構築します。4エリアの範囲（中心X=128/256/384/512、各X±48・Z=-48〜47、地表〜Y144）に加えた建築は上書きされます。別の乱数地形にするにはshから新規ワールドを生成してください。

以下は手作業の元データパックと旧撮影ワールドの説明です。新規エクスポートの4エリアでは、寸法・建物数・剣の本数・配置が変わります。
